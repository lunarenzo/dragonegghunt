...
public class HookManager implements Reloadable {
    ...
    public HookManager(DragonEggHunt plugin) {
        this.plugin = plugin;
    }
    ...
    public void onLoad(AbstractPlugin plugin) {
        for (Hook hook : Hook.values()) {
            try {
                ...
                final AbstractHook hookInstance = hook.getHookClass().getDeclaredConstructor(DragonEggHunt.class).newInstance(plugin);
